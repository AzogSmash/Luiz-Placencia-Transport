...
export default function NavBar() {
  ...
  return (
    <>
      <header style={{
        ...
        <div className="container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 76 }}>
            ...
            {NAV_ITEMS.map(item => {
              ...
              return (
                <Link key={item.href} href={item.href} style={{
                  ...
                  color: active ? 'var(--fg)' : 'var(--fg-muted)', fontWeight: 500,
                  paddingBottom: 4,
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <a href="tel:+33643272173" className="mono nav-phone" style={{ color: 'var(--fg-muted)', fontSize: 12, letterSpacing: '0.06em' }}>
              +33 6 43 27 21 73
            </a>
            ...
            <button
              ...
              style={{
                ...
                cursor: 'pointer', padding: '9px 11px', borderRadius: 'var(--radius)',
      ...
      {mobileOpen && (
        <div onClick={() => setMobileOpen(false)} style={{ position: 'fixed', inset: 0, zIndex: 48 }} aria-hidden />
      )}
      {mobileOpen && (
        <nav style={{
          ...
          borderBottom: '1px solid var(--line-soft)', zIndex: 49,
        }}>
          {NAV_ITEMS.map(item => {
            const active = pathname === item.href
            return (
              <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)} style={{
                display: 'block', padding: '18px 24px', fontSize: 13, fontWeight: 500,
                letterSpacing: '0.12em', textTransform: 'uppercase',
                color: active ? 'var(--accent)' : 'var(--fg-muted)',
                borderBottom: '1px solid var(--line-soft)', transition: 'color 0.18s ease',
              }}>
                {item.label}
              </Link>
            )
